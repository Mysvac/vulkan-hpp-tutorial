...
该语言为图形编程提供了诸多便利功能，例如内置了向量和矩阵等原语类型。
它还包含一系列用于执行叉积、矩阵-向量乘法以及围绕向量反射等操作的函数。
在向量类型方面，以`vec`为前缀，其后的数字表示向量中元素的数量，比如3D位置信息会存储在`vec3`类型中。
对于向量的成员访问，既可以单独访问如`.x`这样的单个分量，也能够通过组合多个分量来创建新的向量。
例如，`vec3(1.0, 2.0, 3.0).xy`这一表达式会生成一个`vec2`类型的向量。
此外，向量的构造函数还支持向量对象与标量值的组合使用，
像`vec3(vec2(1.0, 2.0), 3.0)`这样就可以构造出一个`vec3`类型的向量。
