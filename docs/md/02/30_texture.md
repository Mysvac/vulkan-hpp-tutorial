...
我们曾经说过渲染通道的附件布局转换是通过子通道依赖控制的，而子通道依赖的本质是内存屏障，因此一种最常用的修改图像布局的方式是直接使用图像内存屏障(image memory barrier)。

内存屏障常用于同步资源访问，比如先写后读。
但它也可以在 `vk::SharingMode::eExclusive` 时用来变换图像布局和转移队列族所有权。
