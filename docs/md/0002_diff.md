...
`vk::raii::`类型纯储了非`raii`类型的指针，并在析构时自动调用`Destory`清理资源。 
可能禁用默认构造，但提供`nullptr`构造。可能禁用拷贝，运行移动。

比如`vk::raii::Semaphore`类型，内部就纯储了`vk::Semaphore`类型的指针。

此时会产生问题，很多函数的参数是原始类型，而非`raii`类型。
幸运的时，`raii`类型都重载了类型转换运算符，可以隐式转换成对应的非`raii`类型。

但是依然有部分函数，必须输入原始类型，因为输入`raii`类型是隐式转换次数会超过1次，此时可以显式转换。
大部分`raii`类型提供了`*`运算符重载，会返回内部原始类型的引用。
